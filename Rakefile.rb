...
projects_dir    = "_posts/projects"    # directory for project files
...
desc "Create a new project post in #{projects_dir}"
task :new_project, :title do |t, args|
  ...
  filename = "#{projects_dir}/#{Time.now.strftime('%Y-%m-%d')}-#{title.to_url}.#{new_post_ext}"
  if File.exist?(filename)
    abort("rake aborted!") if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
  end
  tags = get_stdin("Enter tags to classify your post (comma separated): ")
